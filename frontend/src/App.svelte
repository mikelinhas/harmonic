...
<header class="mb-2 flex w-full items-center justify-between">
  <div class="flex flex-col gap-0.5">
    <span class="text-label tracking-caps text-muted uppercase">INPUT TERMINAL</span>
    <span
  ...
  {#if roomCode}
    <Button variant="sm" onclick={() => (appState.showSettings = true)}>⚙ SETTINGS [X]</Button>
  {/if}
</header>
...
>
  {#if !roomCode}
    <!-- Room picker -->
    <div class="flex flex-1 flex-col items-center justify-center gap-7 px-5 py-16 text-center">
      <div
      ...
      <div class="flex w-full max-w-xs flex-col gap-1.5">
        <label class="text-label tracking-caps text-muted text-left" for="room-input">
          ROOM CODE
        </label>
        <input
          class="text-bright font-retro tracking-code placeholder:text-muted w-full border-0 border-b border-[var(--color-green-dim)] bg-transparent py-2 text-sm uppercase outline-none transition-colors duration-200 focus:border-[var(--color-green)]"
          id="room-input"
          type="text"
          placeholder="e.g. SPRINT42"
          autocomplete="off"
          spellcheck="false"
          maxlength="20"
